...
| `EPD462` | VR22 | `FUEL_DELIVERIES` | **SUPERSEDED by WP-17.** Tolerance resolves from `fob_source`, not a single figure: ACARS ±0.5% floor 50 kg; CREW_REPORTED and PANEL_PRESET ±1.5% floor 200 kg. The crew floor is doubled 100 kg rounding — a percentage alone cannot work, since 100 kg on a 400 kg top-up is 25%. An ACARS reading and a crew-reported one cannot be held to the same threshold. Migrates to `TOLERANCE_RULES` unchanged at WP-13 | Error |
| `EPD463` | VR23 | `FUEL_DELIVERIES` | **AMENDED by decision C-1.** A variance raises an exception task. It does **not** block invoice approval or posting. The supplier is paid on metered volume and the dispute runs on its own track — which is why claim windows exist at all: if payment could simply be withheld, a 15-day notification deadline would be pointless. Payment is held only where `HOLD_PAYMENT_ON_DISCREPANCY` is set **and** tolerance is exceeded; the parameter defaults off and is not yet built. The original wording predates the SME review and is withdrawn | Error |
