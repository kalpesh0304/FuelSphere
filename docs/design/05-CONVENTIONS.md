...
| Trap | Detail |
|---|---|
| **Never verify with a case-sensitive grep alone** | Twice a check reported clean while the tool it examined was emitting the opposite. WP-04 grepped for `ConcurrencyMode` when OData v4 emits `Core.OptimisticConcurrency`. WP-08 grepped for `^\[error` when the compiler emits `[ERROR]`. **A verification that can produce a false pass is worse than none** — it converts an unchecked assumption into a recorded one. Key on the exit code, and where a grep is unavoidable, prove the instrument by matching a known-present string first |
