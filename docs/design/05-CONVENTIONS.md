...
| **A transatlantic arrival that never rolls to the next day** | S2 carried `sibt` **eleven hours before `sobt`** — an arrival time computed as a clock value and never advanced a date. **On a seven-hour eastbound sector the arrival is the next calendar day**, and a bare time cannot say so. Every duration computed from that pair is negative, and **nothing rejects a negative duration** because nothing was asked to. **Check that arrival follows departure before computing anything from the interval** |
