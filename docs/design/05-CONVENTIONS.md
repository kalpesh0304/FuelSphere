...
| **Draft enablement is inherited — and a gate fires at ACTIVATION, not at POST** | A composition child of a draft-enabled entity is draft-enabled too, without saying so anywhere. **FOUR distinct ways now:** WP-12 a CREATE that never fired on a child; WP-19 a `.drafts` path on an entity that has none; WP-21A inheritance nobody declared, `InvoiceExceptions` needing `IsActiveEntity` because `Invoices` is; and **S6 a `before CREATE` gate returning 201 on the POST**, because a POST to a draft-enabled collection creates a **draft** and the gate fires on `draftActivate` — at the point the order becomes a commitment, not the point someone starts typing one. **The fourth has the worst consequence:** a harness asserting on the POST alone reports a WORKING gate as ABSENT, **and the code gets changed to fix something that is correct.** Before touching any entity, establish whether it is draft-enabled, including by inheritance |
...
| **Merging the BASE of a stacked PR before the PR itself orphans it** | Package F was based on Package E's branch so its diff showed one package. **E merged to `main` first, and F then merged into a branch that had already been absorbed** — so F landed somewhere nobody would merge again. **Nothing shows a problem:** the PR says merged, the merge commit exists, the branch is fine. **The stranding is visible only from `main`.** Merge the top of a stack first, or **retarget the child the moment its base lands.** It was caught by a check looking elsewhere — S2's verification compares S3's variance as part of a pair and reported the old figure |
| **A transatlantic arrival that never rolls to the next day** | S2 carried `sibt` **eleven hours before `sobt`** — an arrival time computed as a clock value and never advanced a date. **On a seven-hour eastbound sector the arrival is the next calendar day**, and a bare time cannot say so. Every duration computed from that pair is negative, and **nothing rejects a negative duration** because nothing was asked to. **Check that arrival follows departure before computing anything from the interval.** **AND THE DEFECT IS NARROWER THAN “long-haul is wrong”:** the sweep found **four** flights that DO cross a date boundary and **all four are correct.** Only two need the rollover and never got it — S2, and `AC402 NRT→YVR` at `17:30Z → 10:30Z` the same day. **A defect present in two of six cases is findable; a defect described as a class of route is not** |
| **A suite that has never moved has never been tested for whether it can** | Twenty-nine harnesses passed from the scratchpad for weeks and **two failed on the first run from `test/`.** Both shell out to sibling scripts — `code-gate.js`, `wp31-census.sh` — which **are not harnesses, did not match `*harness*.js`, and were not copied.** One then carried two absolute paths of its own. **Three dependencies on its own location, invisible from the only place it had ever run.** A glob that selects the obvious files silently omits what they call. **Derive the repository root from the file's own path, and move the thing once before trusting it** |
...
| **An assertion with an OR is tested on its weakest branch** | S6's EXIT-4 accepted `e.code === 'MDM404' \|\| /PROVISIONAL/.test(e.message)` and **passed on the regex** — the code was wrong and nobody knew, because the message branch was satisfied. The real code is `MDM402`. **Any error mentioning PROVISIONAL would have satisfied it**, including one raised for a different reason entirely. **An OR in an assertion is a claim that either branch is sufficient**, and the weaker one is what the test actually checks. Key on the code alone |
| **A survey answers the question it was given, not the question that matters** | Three instances in one day. **A third projection of `FLIGHT_SCHEDULE` two surveys missed** — a bare `as projection on` carries every association and announces nothing. **Four `PROVISIONAL` rows called unreferenced after querying one table.** And **a whole package specified, argued from data and given a sixth column, for something that already existed** — the survey asked which services expose `FUEL_DELIVERIES` and never asked what `FUEL_ORDERS` declares, which was one line above. **The compiler caught the last one with `Duplicate definition of element`** — but only because it was an element. **A duplicate FACET compiles at exit 0 and ships two identical sections.** Before proposing a thing, read the declaration you would be adding it to |
| **A case that EXISTS may still not be AVAILABLE** | The convention says check whether a case already exists unreferenced before seeding a new master row. It does — four `PROVISIONAL` tails — **and all four are load-bearing**, each carrying a WP-06 scenario: a multi-ticket order, an unmatched ticket, a defuel, a broken ledger chain. **Existing is not the same as free.** Reuse means sharing a tail with another demonstration, which may be fine — a register accumulating history is what a real one looks like — but it is a decision rather than a saving |
| **A harness can assert that a DESIGNED behaviour never happens** | `wp07` and `wp07b` both asserted every registration resolves — which is the assertion that **`ACCEPT_PROVISIONAL` has never fired.** WP-07B designed that branch; the harnesses encoded its **absence** as an invariant, **contradicted the policy for two packages, and nothing noticed** because no seed row exercised it. **Not a pin and not a vacuous check** — a test that passes precisely while a feature is unused, and fails the moment it works. **The invariant was “nothing dangles”, not “everything resolves”.** When a harness asserts a universal, ask whether the design permits an exception — and if it does, the harness is asserting the exception cannot occur |
| **A harness that pins another scenario's data inherits that scenario's corrections** | WP-DEMO-01, WP-18 and WP-19 all pinned S1's rows as their own fixtures, and correcting S1 broke six tests across three merged packages **with no signal until something moved.** The pin is invisible until it fails, and **it fails in the package that changed the data rather than the one that depends on it.** **Two distinct repairs, and telling them apart is the whole skill:** where the criterion still holds and only the number moved, **update the pin.** Where the correction removed the property the test was checking — `wp18 EXIT-1b` asserts additional and extra are never merged, and S1 now zeroes both deliberately — **repoint the fixture, never relax the criterion.** **A test that stops checking its property still passes.** **AND WHEN THE FIXTURE KEEPS MOVING, THE FIXTURE WAS THE WRONG IDEA:** `wp18 EXIT-1b` was repointed three times — AC410, then AC412, then S3 — because each corrected scenario deliberately zeroes `additional` and `extra` for the same reason. A fourth was coming. **The criterion is a property of the DATA, so the test now searches for a qualifying row and fails loudly if none exists.** Seven baseline plans qualify and none is a demo scenario. **A pin that chases every correction is a test coupled to a fixture rather than to its property.** **THE GENERAL FORM, AFTER TWO REMOVALS: a test that asserts today's ANSWER will move every time the answer is corrected; a test that asserts the RELATIONSHIP will not.** `wp18 EXIT-1b` pinned a flight and now searches for a qualifying row. `demo01` hardcoded `309.90` as S2's tolerance and now asserts it is **0.5% of its own metered mass** — and that S1 is a floor case **because 0.5% of its mass falls under 50.** Both derived. **Neither will move again** |
| **A plain `Value:` compiles clean and renders as TEXT, not a link** | The display package wrote a field group entry as `{ Value: tail_registration }`, compiled at exit 0, and emitted **`UI.DataField`** — **a third text column shipped as a navigation.** The mechanism is `UI.DataFieldWithNavigationPath` with a `Target`. **Nothing in the compile distinguishes them**, and the rendered page shows a value either way. **Read the emitted EDMX record type, not the exit code** — `cds compile srv -s all --to edmx-v4` and check what the annotation actually became |
| **Master data can hold variety no transaction exercises** | One survey found three in the aircraft register: **four `PROVISIONAL` rows no flight points at**, **`on_own_aoc` true on all fourteen with zero readers in `srv/`**, and **two rows with a blank `aircraft_type_code`** whose type association dangles. Each is a state the seed can produce and nothing reaches. **Same shape as an unpopulated association or an empty facet** — present, correct, and demonstrating nothing. **Before seeding a new master row to make a case, check whether the case already exists unreferenced** |
| **A managed association can be followed only to its target's KEYS** | So a two-hop `on` condition through one is **refused, not awkward**: *“Can follow managed association `FUEL_DELIVERIES:order` only to the keys of its target, not to `flight`”*. A derived to-one over `order.flight_ID` is refused for the same reason plus *“target entity has no element `flight_ID`”*. **There is no annotation-only path from a flight to its deliveries or tickets** — and the reachable alternatives are both wrong in opposite directions: a tail-plus-date join **over-matches on 5 of 13 pairs**, a two-hop **under-matches** because a ticket with no order vanishes. **One hop from the order is neither.** **AND THE SECOND CLAUSE, MEASURED LATER: a VIEW's select list carries no such restriction** — `order.flight.ID as flight_ID` inside `entity FLIGHT_FUEL_DELIVERIES as select from FUEL_DELIVERIES` resolves fine, and an association then compares plain column to plain column. **So the path exists as a SCHEMA change and never as an annotation.** The 83 lines of view are not optional: without them there is nothing for a facet to target |
| **Annotations do not propagate sideways, and propagating downward is the hazard** | Measured in an isolated model, instrument proved both ways. **`annotate SvcA.Items` reaches `SvcA` and NOT `SvcB`** — sibling projections inherit nothing, so Package B's per-service annotation of `PlanningService.FuelOrders` was **forced rather than chosen**, which nobody recorded. **`annotate` on the DB entity reaches every projection** — no duplication, and **every projection must then expose every field the annotation names, forever, with nothing checking it.** A db-level `LineItem` naming `c` on a projection carrying only `ID, a, b` **compiles at exit 0 and fails at read.** This repository is shaped for it: `PlanningService.FuelOrders` exposes **12 of 52** elements. **A restriction makes a db-level annotation dangerous and a per-service one cheap** — same restriction, opposite effect, depending on where the annotation lives |
...
| **Do not add a second place to hold one fact, especially knowing the pattern** | The workbook offered an `apu_burn_rate_kg_hr` at TYPE level, and `AIRCRAFT_MASTER` has no such field **by design** — the schema says at the site *“never metered; APU burn derives from this”*, on the REGISTRATION, and `apu-burn.js` reads the per-tail rate and nothing else. **Adding the type-level column would create two sources of truth for one fact, able to disagree the moment a tail's rate is corrected.** What makes it worth a row rather than a note: **every other instance of this family was INHERITED from an older decision; this one would have been SELF-INFLICTED**, chosen today with the pattern already catalogued. **The refusal is the finding, not the detection** |
