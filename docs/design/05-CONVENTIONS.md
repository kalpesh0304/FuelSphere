...
| **A read-only report is not a guard** | A check that REPORTS and a check that PREVENTS are indistinguishable from outside — both produce an error code against the record, in the same format, with the same severity. `validateDelivery` returns `EPD401`, `EPD403` and `EPD404` and blocks nothing; the same codes now also fire from a `before` handler that does. **Establish which one you are looking at before concluding a rule is enforced**, and before recording that it is. WP-13 found `delivered_quantity` guarded by nothing but a report, which is D31 |
| **A check that skips what it cannot parse reports clean on less than it claims** | The HDI seed validator **silently skipped 251 columns it could not resolve** — it had not modelled the `AuditTrail` and `ActiveStatus` aspects or association-FK flattening — so those cells were **never type-checked at all** and a clean result would have been partly hollow. Distinct from the instrument traps above: not the wrong tool, and not a check that cannot fail, but **a check that quietly narrows its own scope and reports success on the remainder.** The issue set turned out unchanged at 46, so nothing was hiding — **but that was luck, not method.** **Count what a check SKIPPED, not only what it failed, and drive the skipped count to zero before trusting the result** |
| **Prove an annotation enforces before relying on it — and before reporting that it does not** | `@assert.range` fires on a non-draft write, is **deferred to `draftActivate`** on a draft-enabled entity, never fires on `db.run`, and cannot fire at all on a field carrying `@Common.FieldControl: #ReadOnly` because the value is stripped before validation. WP-13 read the deferral as non-enforcement and reported it, which would have invalidated two merged packages had it been true. **Four hypotheses were eliminated by one control**: the same numeric assertion on an unrelated non-draft entity, which rejected — turning a plausible explanation into a conclusive one |
| **Test the case you care about, not a neighbouring one that happens to pass** | WP-09 and WP-12 both proved `@assert.range` enforces, and both tested **enums**. The criteria were sound; generalising from them to numeric bounds on a draft entity was not, and nothing in either package was wrong. **A passing test licenses the case it ran, and no other** — when a conclusion is about a class, the control has to vary the thing that defines the class |
