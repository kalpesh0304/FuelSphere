...
| **Prove an annotation enforces before relying on it** | `@assert.range` rejects an out-of-enum value and accepts an out-of-numeric-range one, on the same entity, in the same request path. **Two merged packages proved it works and both tested enums.** Neither proved less than it claimed — the criterion was sound and the generalisation was not. **When an annotation is believed to enforce something, test the case you actually care about**, not a neighbouring one that happens to pass |
