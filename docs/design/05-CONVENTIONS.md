...
| **Reading a trap does not prevent it either** | WP-12 wrote a fresh enum sweep that reported 179 false violations by resolving members from the wrong type — the `SECURITY_USERS.employment_status` trap, in this list since WP-06 **and read earlier in the same session.** Knowing it was not enough. The discipline is not "check the list" but, when writing any instrument, **ask how this particular one could pass or fail wrongly.** The list is a prompt for that question, not an answer to it |
