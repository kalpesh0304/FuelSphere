...
| **D50** | **A dangling path inside an annotation record emits no warning; a dangling `annotate … element` does.** `statusCriticality`, `approvalCriticality` and `matchingCriticality` were referenced **six times** in `invoice-fiori-annotations.cds` and existed nowhere — not on `INVOICES`, not on the projection, not in the EDMX. The compiler emitted `Criticality Path="statusCriticality"` **verbatim**, Fiori resolved it to undefined, and the three status columns rendered grey for months. The **same file's** dangling `contract` value help warned on every build, because that is the checked form. **CLOSED for InvoiceService** by the reconcile package — the elements were added as calculated elements and the value help removed — but **the mechanism is repository-wide and eight annotation files exist.** `fim-reconcile-harness` EXIT-4 walks every emitted `Path=` and `AnnotationPath=` on one service; **nothing walks the other fourteen** |
