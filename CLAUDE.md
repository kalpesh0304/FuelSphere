...
| **D23** | **THREE implemented services have no authorisation of any kind.** `authorization.cds` covers 4 of 15 services. `PlanningService` (610 lines), `RefuelerService` (235 lines) and now **`PricingService`** — the last added by WP-20 — have no annotation block, not even a service-level `@requires`. The count has grown once already and will grow again with every service that gains handlers. `PlanningAccess` is defined as a scope and appears in no grant for that reason, which is a missing grant on working code rather than an unimplemented module |
