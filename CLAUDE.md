...
| **D44** | **`FLIGHT_SCHEDULE.fuel_order` is a to-ONE over a to-MANY condition, and handlers read it.** `Association to FUEL_ORDERS on fuel_order.flight = $self` — the condition can match several rows and the association returns **one arbitrarily.** **Measured, not theoretical: PR1041 already carries two orders.** So every handler reading `fuel_order_ID` gets an order on that flight, never *the* order, and which one is not determined. **This is why the object page showed a blank order number** — a cardinality error surfacing as a UI symptom. Retained rather than removed, because handlers depend on it, and **commented at the site**: it means *an order on this flight*. **The real fix is for each reader to say which order it wants**, and until then any behaviour keyed on it is undefined where a flight has more than one. Found building Package C | 
