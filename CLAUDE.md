...
| **D56** | **A PATH THAT IS ENTIRELY CORRECT AND CANNOT BIND — the ninth cause of an empty section, and the first where the ANNOTATION IS RIGHT.** The other eight are about what a path NAMES; this is about what a path can TRAVERSE. `FlightSchedule.station_default/supplier/supplier_name` names three real things and has never shown a value, because `station_default` is an `Association to many` and Fiori has no key for the first hop. **Two symptoms, and the second is the one that reaches a screen:** `GET .../station_default/supplier` returns **404**, and `$select=station_default/supplier/supplier_name` returns **200 with the field present and null, always** — against `$select=tail/registration` returning `"RP-C8801"` on the same rows. **This is the SME's request, asked for twice** — designated supplier as a filter AND as a list column — and it has never worked. **NOT FIXED, because the repair is a decision:** a to-one over `station_code` is D44 exactly, and copying the name onto the flight is what D's *resolved, never copied* rule forbids and `d-designated-suppliers` EXIT-9 asserts against. **The sweep now catches the class** — `d50` EXIT-5, 1,613 paths across 15 services, this one KNOWN and any new one failing. **It cost nothing to add:** cardinality was always in the EDMX as `Type="Collection(…)"` and `parse()` was stripping it to get the target type. The instrument had the evidence and discarded it |
