...
| ~~D16~~ | **CLOSED.** WP-05 removed it; confirmed by the WP-21A closure survey that no `100000` guard remains in any handler |
...
| **D30** | **RESTATED 21 Aug 2026 — the original premise was wrong, and what replaces it is larger.** There were never two enforcers disagreeing. Measured: `@assert.range` **works**, numeric and enum alike — a numeric `[0, 100]` on a non-draft entity rejects out-of-range input. On a **draft-enabled** entity, including a composition child that inherits it, validation is **deferred to `draftActivate`**, which is why the temperature and density assertions looked inert on a child POST and fired on activation. **`db.run` bypasses all of it**, enum and numeric alike, and that is how every handler here writes. **THREE numeric assertions exist in the whole repository** — `FUEL_DELIVERIES.temperature`, `FUEL_DELIVERIES.density` and `ROB_LEDGER.closing_rob_kg`. The first two enforce and are replaced by WP-13 with a store-resolved check, because an annotation cannot read a configuration store. **The third is genuinely inert, and by a SECOND MECHANISM worth naming separately from the draft one:** it carries `@Common.FieldControl: #ReadOnly` on its service projection, so CAP strips the value from input and the assertion never sees one — its `@mandatory` cannot fire either. **An annotation can be inert for more than one reason, and finding the first does not rule out the second.** Deferral to `draftActivate` and read-only stripping are independent, and a field could carry both. **Decision B8 reasoned the ROB clamp might exist to satisfy that assertion. It could not have.** B8 stands regardless, on WP-03's handler, which raises `FB402` at `burn-service.js:632` and withholds the row. Separately, the burn variance ladder was written out three times in two forms and the `ToleranceType` enum had no member for temperature, density or burn variance — both closed by WP-13. Found by the WP-21A closure survey and the WP-13 investigation |
| **D31** | **A negative delivered quantity is accepted today.** `FUEL_DELIVERIES.delivered_quantity` has **no `@assert.range` and no write guard**. Its only check is `EPD401` inside `validateDelivery` at `order-service.js:701`, which **records and returns** — a read-only report that blocks nothing. **This is not an inert assertion and the distinction is operative:** an inert assertion is fixed by removing or replacing it, whereas this needs a guard written. Nothing about the field looks guarded to anyone reading the schema, so it is absence rather than false assurance. WP-17 making the field derived moots it; until then it is open. Found by the WP-13 investigation |
...
| **D23** | **FOUR implemented services have no authorisation of any kind.** `authorization.cds` covers 4 of 15 services. `PlanningService` (610 lines), `RefuelerService` (235 lines), **`PricingService`** (added by WP-20) and **`InvoiceService`** (added by WP-21A) have no annotation block, not even a service-level `@requires`. **The count grows with every service that gains handlers without an `@restrict`** — it has been re-counted three times already (two at WP-02, three at WP-20, four at WP-21A). Treat the number as a symptom; the defect is that nothing requires a service to declare authorisation before it ships behaviour. `PlanningAccess` is defined as a scope and appears in no grant for that reason, which is a missing grant on working code rather than an unimplemented module |
...
| **An annotation may enforce on a path you did not test** | `@assert.range` fires on a non-draft write and is **deferred to `draftActivate`** on a draft-enabled entity — including a composition child that inherits draft without declaring it. WP-13 first read that deferral as non-enforcement. It also never fires on `db.run`, which is how every handler here writes, and it cannot fire at all on a field carrying `@Common.FieldControl: #ReadOnly`, because the value is stripped before validation. **Prove an annotation enforces on the path you care about before relying on it — and before reporting that it does not.** |
