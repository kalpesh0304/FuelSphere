...
| **D30** | **Two thresholds are enforced twice, and config alone will not move them.** Temperature and density limits exist as literals in the handler **and** as `@assert.range` on `db/schema.cds:909,911`. Move the literal into configuration and **the schema assertion still enforces the old value** — config changes, behaviour does not, and nothing says why. **The worst failure mode a parameter store can have.** Separately, the burn variance ladder is **written out three times in two different forms** in `burn-service.js`, so moving "the" constant moves one of three. And `ToleranceType` has **no member** for temperature, density or burn variance, so none of them can be represented as a tolerance rule today. All found by the WP-21A closure survey | 
