...
| **D47** | **Nine to-many associations dangle across five services, and declaring one is not what makes it reachable.** A `to many` produces a navigation property **only where the service exposes both ends** — otherwise the compiler emits *“No OData navigation property generated, target is outside of service”* and the association is declared everywhere and navigable nowhere. **Measured across all five projections of `FLIGHT_SCHEDULE`:** Planning dangles `burns` and `dispatches`; FuelOrder `burns`; Burn `dispatches`; CostAllocation both; Refueler `burns`, `dispatches` **and** `orders`. **Eight of the nine remain** — exposing `FLIGHT_DISPATCH` on PlanningService closed Planning's `dispatches` and nothing else; the measurement is kept as taken rather than restated downward, because the count is what makes the mechanism visible. **So Package C landed three to-manys and only TWO have a service that can carry them** — `orders` renders on PlanningService, and `dispatches` on **FuelOrderService and PlanningService**, each with its own projection and its own annotation — a fuel controller reaching a plan from an order and a planner reaching it from a flight are different journeys, and neither annotation is a copy of the other — and **`burns` renders nowhere, because no service exposes both `FLIGHT_SCHEDULE` and `FUEL_BURNS` navigably.** Originally recorded for one projection on one service; the condition holds on five. **Which service exposes what is the decision, and it is per service** |
