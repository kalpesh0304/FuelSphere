...
| **D47** | **`RefuelerService.FlightSchedule` carries associations that produce no navigation.** A bare `as projection on db.FLIGHT_SCHEDULE` at `refueler-service.cds:107` exposes **both `fuel_order` and `orders`**, and the compiler emits *“No OException navigation property generated, target is outside of service”* for each — six warnings on that line, predating any recent branch. **So the ambiguity D44 fixed cannot even be reached there**, because the association resolves to nothing. Left alone: it is the **supplier's** service, and Package D's boundary reasoning applies — excluding it would change the refueler side to fix something not reachable from it. **A third projection of `FLIGHT_SCHEDULE` that two surveys missed** | 
