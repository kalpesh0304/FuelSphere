...
| ~~D56~~ | **CLOSED — AND THE ROW WAS WRONG ABOUT THE SYMPTOM, WHICH IS WHY IT SURVIVED. The observation held and the inference did not, the shape recorded for D1 and D35.** It said `$select=station_default/supplier/supplier_name` returns *"200 with the field present and null, ALWAYS"*. **Measured on all 22 flights: it returns a NAME on 14 and is blank on exactly the 8 with no designation at all.** The blanks are correct. **`$filter` on that path works too — 12 rows.** So the thing everyone re-checking would have looked for — an empty column — was not there to find, and anyone who did look saw names and moved on. **THE REAL DEFECT WAS WORSE AND QUIETER: the path names the STATION AXIS BY CONSTRUCTION** (`station_default`'s `on` carries `flight_number is null`), so a flight-level designation can never reach it. **On AC102 the column showed BP Aviation United Kingdom while the designated supplier is Air Total International** — and the filter made the same error twice over: `eq 'Air Total'` **MISSED** AC102, `eq 'BP Aviation'` **RETURNED** it. **A blank column sends a planner to look; a column headed with the right name showing the wrong supplier does not.** It is wrong on precisely the flight that has a specific arrangement, which is the only kind anybody made an arrangement FOR. **FIXED by `FLIGHT_DESIGNATED_SUPPLIER`** (`db/flight-designated-supplier.cds`) — at most one row per flight, reached by a to-ONE. **Not the D44 shape this row warned against:** that was a to-one over a condition matching MANY; this target holds one row **by construction**, because **where two designations compete at the same axis the view emits NOTHING rather than choose.** It reproduces the scope, the date window and the axis (two lines, and not shared code in the resolver either — it is query order there); it does **not** reproduce specificity-then-priority, which stays in `parameter-store`. **The invariant that buys: the view AGREES with `resolveDesignation()` or it says nothing — it never shows a different supplier.** A blank is recoverable because somebody asks why. **The decline arm is UNEXERCISED BY DATA** (0 of 22 flights reach a same-axis tie) and is proved by a plant, not by a row. **`d50` EXIT-5's KNOWN set is now EMPTY, and the ratchet's own stale-check is what said so** — it fired the moment the column was rebound, which is the check earning its place: an accepted list nobody re-derives becomes a record of decisions nobody took |
