...
| **D41** | **`ACARS_DERIVED` was specified and never added, and the burn derivation needs it.** `01-TARGET-SCHEMA` §535 extends `FobSource` to five members; the schema has **four**. **It is the one directive in that document not met.** Two things depend on a delivery being able to say its readings were **derived rather than measured**: §551, where `ground_burn_kg` inverts on the derived path, and §493, deriving the gauge uplift from `OUT − IN` plus APU. **WP-19 built the derivation; the value that marks its output is missing**, so a derived figure and a measured one are indistinguishable — and they carry different tolerances. Also: the burn variance ladder is written **three times in two forms** in `burn-service.js`, carried over from the closed D30 | 
