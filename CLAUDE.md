...
| **D43** | **`closure_document` is exposed on `PlanningService` and its target is not.** WP-31 added the association on `FLIGHT_SCHEDULE`; `PlanningService` projects it, but `SOURCE_DOCUMENTS` is **not exposed on that service.** So the tech-log image **cannot be reached from the planning screen** even though `closure_source` reads `OCR` there — the flight says its closure time was read from a photograph and offers no way to see it. Either expose `SourceDocuments` on `PlanningService`, or accept that closure evidence is reachable only from the services that carry it and say so. **Found while walking the nine screens** | 
