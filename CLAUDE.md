...
| Aircraft and schedule fields the 1 Sept brief assumed exist | **Measured, and three do not.** `MLW`, `MZFW` and `engine_burn_rate_kgph` are **nowhere in the model** — not on the tail, not on the type — so work package B is half migration and half research. `MTOW` exists **on `AIRCRAFT_MASTER` (the type), not on the registration**, and is populated 17/17. And **`terminal` does not exist on `FLIGHT_SCHEDULE`** although the brief lists gate, stand and terminal as present: gate and stand do exist, terminal was generalised from them. **A field that is not there is a question; a blank one is the eighth cause of an empty section** — so A's aircraft block ships these MISSING rather than blank |
