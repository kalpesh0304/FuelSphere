...
annotate FuelOrderService.FuelOrders with @(
    ...
    UI: {
        ...
        Identification  : [
            // CAPTURE A TICKET FROM THE ORDER IN FRONT OF YOU.
            //
            // In UI.Identification because that is where both conventions in
            // this repository put an object-page action, and because NOTHING
            // HERE CAN RENDER A PAGE TO CHECK ANY OTHER FORM: $fiori-preview
            // bootstraps from ui5.sap.com, which is 403 in the build
            // container, so its 200 is a route resolving and no pixels. The
            // form with local precedent is the only defensible choice.
            //
            // Not critical and not guarded: decision A1, the fuel is in the
            // tanks before a ticket exists. `submit` below carries
            // IsActionCritical because it commits money; this records what
            // already happened.
            {
                $Type : 'UI.DataFieldForAction',
                Action : 'FuelOrderService.createFuelTicket',
                Label : 'Capture Fuel Ticket',
                ![@UI.Importance] : #High
            },
            {
                $Type : 'UI.DataFieldForAction',
...
annotate FuelOrderService.FuelDeliveries with @(
    UI: {
        ...
        LineItem: [
            ...
            // No toolbar DataField here for now - removed pending the
            // field-level @Common.SemanticObject link approach, being
            // tested first on Fuel Tickets only.
        ],
...
annotate FuelOrderService.FuelTickets with @(
    UI: {
        ...
        LineItem: [
            ...
            // No toolbar DataField here. The button-based intent navigation
            // was found to resolve the wrong row's context in nested/
            // CollectionFacet-wrapped embeds (returns zero or wrong results).
            // Replaced with a field-level @Common.SemanticObject link on
            // ticket_number below - test case for this pattern.
        ],
...
annotate FuelOrderService.FuelTickets with {
    ...
    ID                  @UI.Hidden;
    // Field-level cross-app link (test case). Renders ticket_number as a
    // clickable link, no toolbar button. Row context is whichever record
    // this field is actually bound to, sidestepping the context-resolution
    // bug seen with the table-level DataFieldForIntentBasedNavigation.
    ticket_number       @title: 'Ticket Number' @mandatory
                        @Common.SemanticObject: 'fueltickets'
